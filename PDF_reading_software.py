# The Following packages/modules are to be imported for Text- to - Speech Conversion
...
# Here we would get the starting and ending page numbers which one would want to read
def get_text(value):
...
def main():
    ...
    # All the required details inside the window
    ...
    while True:
        ...
        # Here the path of the pdf file is READ
        pdf_to_read = values[0]

        if event in (None, 'Cancel'):	# If the User chooses to close the window Or Cancel their choice
            print("Exiting")
            window.close()
            exit()
    ...
    # Here, permission is given to delete the folder if it already exists, where we intend to save our PDF images and
    # audio
    ...
    # Here we load the image(s) created in Text_to_speech folder and read the text in image
    # via pytesseract Optical Character Recognition (OCR) software
    ...
    for file in os.listdir(final_directory):    
        ...
        data = data.replace("|","I")
        #Sometimes the image to text translation would put | instead of the letter I. So we replace | with I
        data = data.split('\n')
        mytext.append(data)
    ...
    # Here we load the audio file in the  Text_to_speech_software folder
    # and play the mp3 file in the folder
    mixer.init()
